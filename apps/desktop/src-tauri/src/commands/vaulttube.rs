...
use crate::services::vaulttube::{
    SearchResult, VaultTubePlaylist, VaultTubeRepository, VaultTubeSubscription, VaultTubeSync,
    VaultTubeVideo, UserPlaylist, UserPlaylistItem
};
...
/// Ajoute un abonnement : sonde les métadonnées via yt-dlp, enregistre,
/// puis première synchro des vidéos. Commande longue (le frontend
/// affichera un état de chargement).
#[tauri::command]
pub fn vaulttube_add_subscription(
    ...
    let (name, youtube_id, kind, thumbnail) = sync.probe_url(&url)?;
    let id = repo.add_subscription(&name, &url, &kind, &youtube_id, thumbnail.as_deref())?;
    ...
    // Première synchro — non bloquante en cas d'échec.
...
pub fn vaulttube_refresh_subscription(
    ...
    if sub.thumbnail_url.is_none() {
        if let Ok((_name, _id, _kind, thumb)) = sync.probe_url(&sub.url) {
            if let Some(t) = thumb {
...
pub fn vaulttube_search(
    ...
    VaultTubeSync::new(VaultTubeRepository::new(state.db_pool.clone())).search(&query)
}
...
pub fn vaulttube_create_user_playlist(
    ...
    VaultTubeRepository::new(state.db_pool.clone()).create_user_playlist(&name)
}
...
pub fn vaulttube_remove_from_user_playlist(
    ...
    VaultTubeRepository::new(state.db_pool.clone()).remove_user_playlist_item(playlist_id, &youtube_id)
}
