...
pub fn start_share(app: &AppHandle, path: &str, lan_only: bool) -> Result<ShareOffer, String> {
    if active_slot().lock().unwrap_or_else(|e| e.into_inner()).is_some() {
    ...
    *active_slot().lock().unwrap_or_else(|e| e.into_inner()) = Some(ActiveShare {
...
fn serve_loop(
    ...
    *active_slot().lock().unwrap_or_else(|e| e.into_inner()) = None;
}

pub fn stop_share() {
    if let Some(session) = active_slot().lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
